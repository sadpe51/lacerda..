...
-- RIGHT (routeB)
local stealPath1 = {
    {pos=Vector3.new(-473.9, -7, 27.3)},   -- R1
    {pos=Vector3.new(-484.1, -5.1, 28.4)}, -- R2
    {pos=Vector3.new(-473.9, -7, 27.3)},   -- R3
    {pos=Vector3.new(-475.1, -7, 98.2)},   -- R4
}

-- LEFT (routeA)
local stealPath2 = {
    {pos=Vector3.new(-475.4, -7, 93.5)},   -- L1
    {pos=Vector3.new(-484, -5.1, 92.4)},   -- L2
    {pos=Vector3.new(-475.5, -6.9, 93.5)}, -- L3
    {pos=Vector3.new(-473.5, -7, 17.1)},   -- L4
}
...
local function espCreateHighlight(character)
    local h = Instance.new("Highlight")
    h.Name = "LH_PlayerESP_Highlight"
    h.Adornee = character
...
local function espCreateBillboard(character, plr)
    local hrp = character:FindFirstChild("HumanoidRootPart")
    if not hrp then return nil end
    local bb = Instance.new("BillboardGui")
    bb.Name = "LH_PlayerESP_Name"
    bb.Adornee = hrp
    bb.AlwaysOnTop = true
...
local function espAttach(plr)
    if plr == player then return end
    local function apply(character)
        if PlayerESP.Highlights[plr] then pcall(function() PlayerESP.Highlights[plr]:Destroy() end) end
        if PlayerESP.Billboards[plr] then pcall(function() PlayerESP.Billboards[plr]:Destroy() end) end
        PlayerESP.Highlights[plr] = espCreateHighlight(character)
        PlayerESP.Billboards[plr] = espCreateBillboard(character, plr)
    end
    if plr.Character then apply(plr.Character) end
    local conn = plr.CharacterAdded:Connect(apply)
    table.insert(PlayerESP.Connections, conn)
end

local function espRemove(plr)
    if PlayerESP.Highlights[plr] then pcall(function() PlayerESP.Highlights[plr]:Destroy() end); PlayerESP.Highlights[plr] = nil end
    if PlayerESP.Billboards[plr] then pcall(function() PlayerESP.Billboards[plr]:Destroy() end); PlayerESP.Billboards[plr] = nil end
end

local function enableESP()
    if PlayerESP.Enabled then return end
    PlayerESP.Enabled = true
    for _, p in ipairs(Players:GetPlayers()) do espAttach(p) end
    table.insert(PlayerESP.Connections, Players.PlayerAdded:Connect(function(p) espAttach(p) end))
    table.insert(PlayerESP.Connections, Players.PlayerRemoving:Connect(function(p) espRemove(p) end))
end
...
-- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
--  DROP BRAINROT (NOVO - WALK FLING)
-- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

local _dropConns = {}
local _dropActive = false

local function startDropBrainrotLoop()
    _dropActive = true
    table.insert(_dropConns, RunService.Stepped:Connect(function()
        if not _dropActive then return end
    ...
    coroutine.resume(co)
    table.insert(_dropConns, co)
end

local function stopDropBrainrotLoop()
    _dropActive = false
...
local function createSmallGUI(name, yPosition)
    ...
    Stroke.Color = Color3.new(0, 0.784, 1)
    Stroke.Thickness = 2
    ...
    Button.Text = ""
    Button.TextColor3 = Color3.new(0, 0.784, 1)
...
local function startDropBrainrot()
    ...
    startDropBrainrotLoop()
end

local function stopDropBrainrot()
    stopDropBrainrotLoop()
end
...
ProgressBarFill.BackgroundColor3 = Color3.new(0, 0.784, 1)
...
local function createLacerdaStylePanel()
    ...
    local function Sld(lbl, mn, mx, init, order, onChange)
        ...
        thStroke.Color = Color3.new(0, 0.784, 1); thStroke.Thickness = 2
    ...
    local _, espBtn, espKn, espTr = Tog("Player ESP", "Destaca jogadores", 18)
    espBtn.MouseButton1Click:Connect(function()
        if PlayerESP.Enabled then disableESP(); SetOff(espKn, espTr)
        else enableESP(); SetOn(espKn, espTr) end
    end)
    Sep(21)
    ...
    -- Mini painel de speed
    local apCfgRow = Instance.new("Frame", Scroll)
    apCfgRow.Size = UDim2.new(1,0,0,44); apCfgRow.BackgroundTransparency = 1
    apCfgRow.LayoutOrder = 24; apCfgRow.ZIndex = 5
    local function makeSpeedBox(label, xPos, getValue, setValue)
        local lbl = Instance.new("TextLabel", apCfgRow)
        lbl.Size = UDim2.new(0,80,0,16); lbl.Position = UDim2.new(0,xPos,0,2)
        lbl.BackgroundTransparency = 1; lbl.Text = label
        lbl.TextColor3 = Color3.new(0, 0.784, 1); lbl.Font = Enum.Font.GothamBold
        lbl.TextSize = 10; lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.ZIndex = 6
        local box = Instance.new("TextBox", apCfgRow)
        box.Size = UDim2.new(0,70,0,22); box.Position = UDim2.new(0,xPos,0,18)
        box.BackgroundColor3 = Color3.fromRGB(20,20,28); box.BorderSizePixel = 0
        box.Text = tostring(getValue()); box.TextColor3 = Color3.new(1,1,1)
        box.Font = Enum.Font.GothamBold; box.TextSize = 11; box.ClearTextOnFocus = false; box.ZIndex = 6
        Instance.new("UICorner", box).CornerRadius = UDim.new(0,5)
        local st = Instance.new("UIStroke", box); st.Color = Color3.fromRGB(0, 180, 220); st.Thickness = 1
        box.FocusLost:Connect(function()
            local n = tonumber(box.Text)
            if n and n > 0 then setValue(n); box.Text = tostring(n)
            else box.Text = tostring(getValue()) end
        end)
    end
    makeSpeedBox("Speed Ida", 10, function() return PATH_VELOCITY_SPEED end, function(v) PATH_VELOCITY_SPEED = v end)
    makeSpeedBox("Speed Volta", 100, function() return PATH_SECOND_SPEED end, function(v) PATH_SECOND_SPEED = v end)

    apBtn.MouseButton1Click:Connect(function()
        if pathActive then stopAutoPlay(); SetOff(apKn, apTr)
...
function createOpenButton(device)
    ...
    MPanel.Size = UDim2.new(0, 250, 0, 480)
    ...
    Sec_m("VISUAL", 25)
    local _, espBtn_m, espKn_m, espTr_m = Tog_m("Player ESP", "Destaca jogadores", 26)
    espBtn_m.MouseButton1Click:Connect(function()
        if PlayerESP.Enabled then disableESP(); SetOff_m(espKn_m, espTr_m)
        else enableESP(); SetOn_m(espKn_m, espTr_m) end
    end)
    Sep_m(27)
